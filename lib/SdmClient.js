...
class SdmClient {
        ...
        try {
          ...
          if (!r.ok) {
            ...
            if (r.status === 400 && /invalid_grant/i.test(txt)) {
              ...
              throw new Error('invalid_grant — 구글 재승인 필요');
            }
            throw new Error(`토큰 갱신 실패 HTTP ${r.status}: ${txt}`);
          }
          // v2.1.2 — 파싱 실패 시 V8 SyntaxError가 body 앞부분(=access_token 단편)을 인용해
          // 폴링 warn까지 전파될 수 있는 이론적 유출 경로 차단 (효율·보안 리뷰)
          let j;
          try { j = JSON.parse(r.body); }
          catch (_) { throw new Error('토큰 응답 파싱 실패 (본문 비출력)'); }
  ...
  async _call(path, opts = {}, _retried = false) {
    ...
    if (r.status === 401) {
      // v2.1.1 — 이 요청에 쓴 토큰이 아직 현재 토큰일 때만 폐기: 동시 요청 A가 방금
      // 재발급한 새 토큰을 B의 낡은 401이 도로 폐기하던 낭비(재발급 왕복) 방지
      if (this._tok === tok) this._tok = null;
